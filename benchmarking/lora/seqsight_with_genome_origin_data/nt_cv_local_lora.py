...
parser.add_argument('--dataset_name', type=str, help='Name of the dataset for fine-tuning', required=True)
...
def get_training_args(fold):
    return TrainingArguments(
        f"checkpoints/lora_32_8_{model_name}-finetuned-{dataset_name}-fold{fold}",
...
for fold, (train_idx, val_idx) in enumerate(kf.split(train_sequences)):
    ...
    lora_config = LoraConfig(
        ...
        bias="none"          # Whether to tune biases ("none", "all", or "lora_only")
    )
...
output_file = os.path.join(output_dir, f"lora_32_8_{model_name}_results_{dataset_name}.csv")
